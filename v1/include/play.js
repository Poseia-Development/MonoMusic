...
module.exports = {
  async play(song, message) {
    ...
    try {
      var playingMessage = await queue.textChannel.send(`🎶 Çalmaya başlandı: **${song.title}** ${song.url}`);
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        case "⏭":
          ...
          queue.textChannel.send(`${user} ⏩ Şarkı atlandı`).catch(console.error);
          collector.stop();
          break;
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            queue.textChannel.send(`${user} ▶ Müzik devam ediyor!`).catch(console.error);
          }
          break;
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Döngü şuan ${queue.loop ? "**açık**" : "**kapalı**"}`).catch(console.error);
          break;
